...
BatchEffectM = list()
KNNtest <- function(exprDat_norm,BioMeta){
...
combat_all_batch9 <- sva::ComBat(t(MySomaAllDat), tranche, mod=mod1, par.prior = TRUE, prior.plots = FALSE)
...
Sys.time()
combat_all_batch10 <- sva::ComBat(t(log(MySomaAllDat)), tranche, mod=mod1, par.prior = TRUE, prior.plots = FALSE)
TrancheEffect10 = KNNtest(t(combat_all_batch10),MetaRaw) 

###11. source: combined(tranche1,2)); combat tranche + plate as covariate; nonparametric
Sys.time()
combat_all_batch11 <- sva::ComBat(t(MySomaAllDat), tranche, mod=mod1, par.prior = FALSE, prior.plots = FALSE)
...
Sys.time()
combat_all_batch12 <- sva::ComBat(t(log(MySomaAllDat)), tranche, mod=mod1, par.prior = FALSE, prior.plots = FALSE)
...
combat_all_batch13 <- sva::ComBat(t(MySomaAllDat), PlateBatch, mod=mod2, par.prior = TRUE, prior.plots = FALSE)
...
Sys.time()
combat_all_batch14 <- sva::ComBat(t(log(MySomaAllDat)), PlateBatch, mod=mod2, par.prior = TRUE, prior.plots = FALSE)
...
Sys.time()
combat_all_batch15 <- sva::ComBat(t(MySomaAllDat), PlateBatch, mod=mod2, par.prior = FALSE, prior.plots = FALSE)
...
Sys.time()
combat_all_batch16 <- sva::ComBat(t(log(MySomaAllDat)), PlateBatch, mod=mod2, par.prior = FALSE, prior.plots = FALSE)
...
legend("right",legend=c("Control","Injury","OA","RA"),col=c("black","red","green","blue"),cex=0.3)
...
BatchEffectM2 = matrix(unlist(BatchEffectM),nrow=5,ncol=6,byrow=TRUE)

combat_all = t(combat_all_batch6)
batchCorrectionList=list(RawM1,RawM2,MySoma1Done,MySoma2Done,combat_all)
