...
### use the BioMetaM (genereated on 18 AUG 2021)
colnames(BioMetaM)
dim(BioMeta1Done)
dim(BioMeta2Done)

levels(as.factor(BioMeta1Done$diseaseGroup))
length(which(BioMeta1Done$diseaseGroup=="OA")) ###261
length(which(BioMeta1Done$diseaseGroup=="Injury")) ###174

levels(as.factor(BioMeta2Done$diseaseGroup))
length(which(BioMeta2Done$diseaseGroup=="OA")) ###528
length(which(BioMeta2Done$diseaseGroup=="Injury")) ###72
length(which(BioMeta2Done$diseaseGroup=="Control")) ###3
length(which(BioMeta2Done$diseaseGroup=="RA")) ###5

### analystes repeats
length(unique(BioMeta1Done$STEpUPID[!is.na(BioMeta1Done$STEpUPID)]))
length(BioMeta1Done$STEpUPID[!is.na(BioMeta1Done$STEpUPID)])
length(unique(BioMeta2Done$STEpUPID[!is.na(BioMeta2Done$STEpUPID)]))
length(BioMeta2Done$STEpUPID[!is.na(BioMeta2Done$STEpUPID)])

length(unique(BioMetaM$STEpUPID[!is.na(BioMetaM$STEpUPID)]))
length(BioMetaM$STEpUPID[!is.na(BioMetaM$STEpUPID)])

# Wrong history
# ## Tranche1 sample type breakdown
# myFilePath <- "/Users/ydeng/Documents/QCstepOA/"  
...
# length(which(duplicated(SampleDat222)==TRUE)) ###32 -->46
